...
void Viewer::drawContents() {
   ...
   if(!textures_) shader_.drawIndexed(GL_TRIANGLES, 0, mesh_->get_number_of_face());
    ...
    if(textures_)
    {
        ...
        shaderTexture_.setUniform("MV", mv);
        shaderTexture_.setUniform("P", p);
        shaderTexture_.setUniform("imag", 1);
        shaderTexture_.drawIndexed(GL_TRIANGLES, 0, mesh_->get_number_of_face());
    }
...
bool Viewer::resizeEvent(const Vector2i &size) {
    texture_window_->setPosition(Vector2i(this->size()[0], 0) - Vector2i(texture_windows_size_, 0));
    return true;
}
...
Viewer::Viewer() : nanogui::Screen(Eigen::Vector2i(1024, 768), "DGP Viewer") {
    ...
    b = new Button(window_, "Texture");
    b->setFlags(Button::ToggleButton);
    b->setChangeCallback([this](bool textures) {
        this->textures_ =! this->textures_;
        if(!textures){
            texture_window_->setVisible(false);
        }
        else{
            texture_window_->setVisible(true);
        }
    });
    ...
    mesh_ = new mesh_processing::MeshProcessing("../data/bad_max.obj");

    texture_window_ = new Window(this, "TextureViewer");
    nanogui::Vector2i size_text_win = Vector2i(texture_windows_size_, texture_windows_size_);
    texture_window_->setPosition(Vector2i(this->size()[0], 0) - Vector2i(texture_windows_size_, 0));
    texture_window_->setSize(size_text_win);
    texture_widge_ = new Texture2DViewer(texture_window_, mesh_);
    texture_window_->setVisible(false);
    textures_ = false;
    performLayout();
