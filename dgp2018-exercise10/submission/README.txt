...
Implementation Comments:
    ...
    Exercise 1.3:
        We tried both approaches:
            1. Solving the simple (n+m)x(n+m) system having the one-hot encoded rows for boundary vertices and
        ...
        However, comparing the runtime shows that the overall runtime of the reduced system is slower as we need
        more time to set up the matrices and need to perform a mapping from vertex to matrix indicies.
        On the Max Planck mesh the first algorithm (1.) needs ~85ms in release profile, while the reduced
...
Results:
    ...
    Comparison of Uniform and Cotangent Weighted Minimal Surfaces:
        ...
        On the other hand, the folder "minimal_cotan" shows the results of applying the minimal surface operation
        multiple times using cotan weights. We observe that the solution is not stationary as the system of equations
        depends not only on the graph structure of the mesh, but also on the geometry. Additionally, after the 4th
        application a strange singularity appears, where the cylinder has zero diameter everywhere except at the
        boundary circles. Iterating even longer results in a periodic behaviour yielding the results shown in
        "after_n.png" and "after_n+1.png" alternatingly.
...
Additionally:
    ...
    We tried the parametrization algorithm also on a Moebius strip model as its geometry is interesting. The Moebius
    strip has only a single boundary. Just because it's cool and the parameter domain looks beautiful, we added the
    screenshot in moebius.png. :)
