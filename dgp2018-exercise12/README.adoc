...
However, with uniform remeshing there are some problems with certain parts like the ears even for a target length of 0.25.
...
Our model has quite a large boundary, but the connection between the bench and the fox is quite narrow and as the mapping is convex, the complete fox will be mapped to a small region in parameter space.
Therefore, the texture looks quite good for the bench, but the quality is poor for the fox.
Our model has no disk topology and we therefore expect edge intersections in the parameter domain.
