...
void Viewer::select_point(const Eigen::Vector2i &pixel) {
    ...
    Eigen::Vector3f origin = nanogui::unproject(Eigen::Vector3f(pixel[0], pixel[1], 0), view * model, projection,
                                                mSize);
    Eigen::Vector3f endpoint = nanogui::unproject(Eigen::Vector3f(pixel[0], pixel[1], 1), view * model, projection,
                                                  mSize);
    ...
    Eigen::Vector3f closest_vertex = mesh_->get_closest_vertex(origin, direction,
                                                               contraint_indices_[edit_constraint_index_]);
    mesh_->set_selection(closest_vertex, edit_constraint_index_);
    cout << contraint_indices_[0] << " " << contraint_indices_[1] << " " << contraint_indices_[2] << " "
         << contraint_indices_[3] << endl;
}
...
Viewer::Viewer() : nanogui::Screen(Eigen::Vector2i(1024, 768), "DGP Viewer") {
    ...
    b->setCallback([this]() {
        if (mesh_->shifted_faces_points_.size() > 0) {
            ...
            surface_mesh::Point q = surface_mesh::Point(displacementXTextBox->value(), displacementYTextBox->value(),
                                                        displacementZTextBox->value());
...
void Viewer::refresh_selection() {
    shaderSelection_.bind();
    Eigen::Matrix<uint32_t, Eigen::Dynamic, Eigen::Dynamic> indices = Eigen::Matrix<uint32_t, Eigen::Dynamic, Eigen::Dynamic>(
            4, 1);
    indices << 0, 1, 2, 3;
...
void Viewer::refresh_selected_faces() {
    ...
    if (fixed_faces_ && mesh_->fixed_faces_points_.size() > 0) {
        ...
        Eigen::Matrix<uint32_t, Eigen::Dynamic, Eigen::Dynamic> indices = Eigen::Matrix<uint32_t, Eigen::Dynamic, Eigen::Dynamic>(
                3, mesh_->fixed_faces_points_.size() / 3);
    ...
    if (shifted_faces_ && mesh_->shifted_faces_points_.size() > 0) {
        ...
        Eigen::Matrix<uint32_t, Eigen::Dynamic, Eigen::Dynamic> indices = Eigen::Matrix<uint32_t, Eigen::Dynamic, Eigen::Dynamic>(
                3, mesh_->shifted_faces_points_.size() / 3);
